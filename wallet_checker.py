...
def local_utc(timestamp):
    # Convert timestamp to UTC
    utc_datetime = datetime.utcfromtimestamp(timestamp).replace(tzinfo=timezone.utc)

    # Define UTC+3.5 time zone offset
    utc_offset = timedelta(hours=3, minutes=30)

    # Calculate local time (UTC+3.5)
    local_time = utc_datetime + utc_offset
...
if starter_page == 1:
    form_wallet = input("---> Get a list of '\033[93mERC20 - Token Transfer Events' by Address\033[0m <---\n\n1. Please enter the target \033[93mWallet Address\033[0m: ")
    ...
    if response.status_code == 200:
        ...
        for result in response_result:
            ...
            transactions_list.append(local_utc(timeStamp_utc).strftime('%Y-%m-%d %H:%M:%S')+ " | Txn Hash: "+ hash +  
    ...
    if option_item == 1:
        founded = []
        os.system('clear')
        search_term = str(input("\033[93mEnter the search term: \033[0m")).lower()
        for index, entry in enumerate(transactions_list):
                 if search_term in entry.lower():
                    founded.append(index)
        file_path = "/root/test/" + search_term + ".txt"
        ...
        os.system('clear')
        file_path = "/root/test/" + form_wallet + ".txt"
...
if starter_page == 2:
    file_path = input("---> Get a list of \033[93m'ERC20 - Token Transfer Events'\033[0m by Addresses <---\n\n1. Please enter the file path containing wallet addresses: ")
    os.system('clear')
        ...
        for value in csv_reader:
            ...
            if response.status_code == 200:
                ...
                for result in response_result:
                    ...
                    transaction_entry = (
                        local_utc(timeStamp_utc).strftime('%Y-%m-%d %H:%M:%S') +
                        " | Txn Hash: " + hash +
                ...
                file_path_wallet = f"/root/test/wallets/{wallet}.txt"
    ...
    option_item = int(input(
        ...
        f" Transactions Collected. <---\n\033[93mProcessing Time: {processing_time} mintues.\033[0m\n\n\033[0m1. Search your term in Transactions\n2. Export Transactions\n\n[?] Enter Number: "
    ))
    ...
    if option_item == 1:
        ...
        os.system('clear')
        search_term = str(input("\033[93mEnter the search term: \033[0m")).lower()
        result_file_path = f"/root/test/search_{search_term}.txt"
        ...
        for wallet in all_transactions.keys():
            print(f"/root/test/wallets/{wallet}.txt")

        merged_file_path = "/root/test/transactions.txt"
